...
/*   Updated: 2025/06/19 12:59:48 by mfernand         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	exec_first_command(t_shell *shell)
{
	...
	if (!(full_path = find_path(shell->cmd_list->name, envp)))
	...
	if (pid == 0)
	{
		if (shell->pipeline.n_pipes > 0)
			dup2(shell->pipeline.pipefd[0][1], STDOUT_FILENO); //besoin de gerer le cas d erreur
...
static int	exec_last_command(t_shell *shell)
{
	...
	{
		if (shell->pipeline.n_pipes > 0)
			dup2(shell->pipeline.pipefd[last][0], STDIN_FILENO); //besoin de gerer le cas d erreur
		close_all_pipes(shell);
		handle_redirections(shell->cmd_list);
		if (is_builtin(shell->cmd_list->name))
			exit(exec_built_in(shell));
